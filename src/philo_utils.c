...
/*   Updated: 2024/08/30 17:27:09 by lscarcel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	pthread_mutex_lock(&philos->table->end_lock);
	long long int current_time;
	current_time = get_time();
	while (get_time() - current_time < time && philos->table->end_simulation != 1)
		usleep(100);
	pthread_mutex_unlock(&philos->table->end_lock);
}
